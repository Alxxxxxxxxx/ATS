...
$queryt = mysqli_query($conn,"select * from unemployed where fullname = '$get_id'")or die(mysqli_error());																								
$count = mysqli_num_rows($queryt);
if ($count > 0){
		mysqli_query($conn,"update unemployed set reason = '$reason',seek = '$seek',doing = '$doing',desire = '$desire',consider ='$consider',finance ='$finance' where fullname = '$get_id'")or die(mysqli_error());
		?>
<script>
...
{
mysqli_query($conn,"insert into unemployed (fullname,reason,seek,doing,desire,consider,finance) values('$get_id','$reason','$seek','$doing','$desire','$consider','$finance')")or die(mysqli_error());
mysqli_query($conn,"delete from employed_data where fullname = '$get_id'")or die(mysqli_error());
mysqli_query($conn,"delete from selfemployed where fullname = '$get_id'")or die(mysqli_error());
